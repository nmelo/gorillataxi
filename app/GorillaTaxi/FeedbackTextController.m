...
- (void)loadView {
    ...
    shareFacebookLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:17];   
...
- (void)submitWasClicked {
	...
	if ([feedbackContentTextView.text length] > 0) {
        ...
        _feedback.user_id = [NSNumber numberWithInt:17];
        
        //Need to nullify the positive and negative vote counts so the Manager doesn't send it. 
        _feedback.positive_vote_count = nil;
        _feedback.negative_vote_count = nil;
        
        assert(_feedback.feedback_type > 0);
