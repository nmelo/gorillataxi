...
- (void)layoutSubviews {
    ...
    self.textLabel.frame = CGRectMake(15,10,281, 40);
    self.textLabel.textColor = [UIColor blackColor];
    self.textLabel.numberOfLines = 2; 
    
    authorLabel.backgroundColor = [UIColor clearColor];
    authorLabel.font = [UIFont fontWithName:@"HelveticaNeue" size:10];
    authorLabel.text = poster.first_name;;
    
    goodNumberLabel.backgroundColor = [UIColor clearColor];
    goodNumberLabel.text = [NSString stringWithFormat:@"%@", ((FeedbackTableItem*) _item).feedbackObject.positive_vote_count ];
    badNumberLabel.backgroundColor = [UIColor clearColor];
    badNumberLabel.text = [NSString stringWithFormat:@"%@", ((FeedbackTableItem*) _item).feedbackObject.negative_vote_count];
...
-(void) goodButtonWasClicked:(id)sender {
    ...
    //Trigger delegate to inform the table that it needs to reload. 
    if ([self.delegate respondsToSelector:@selector(userVotedOnCell:)]) {
        [self.delegate userVotedOnCell:self];
    }
...
- (void)setObject:(id)object {
	if (_item != object) {
        ...
        //Not sure why the object would ever be nil?
        if(object != nil) {
            self.delegate = _item.delegate; 
        }
