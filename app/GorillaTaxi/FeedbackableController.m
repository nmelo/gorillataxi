...
- (void)didLoadModel:(BOOL)firstTime {
    ...
    if ([self.model isKindOfClass:[RKRequestTTModel class]]) {
        ...
        for (DBFeedback* _feedback in model.objects) {
            ...
            //We are going to take advantage of the delegate for the TableItem, that is not being used right 
            //now to pass the TableView controller down the hierarchy to the TableViewCell. 
            item.delegate = self;
            [item.delegate retain];
            
            //This invalidates the navigation when the cell is clicked. 
            item.URL = nil;
...
- (void) userVotedOnCell:(id)sender {
    ...
    //This tableViewController subscribes to Vote Up and Down notifications from the FeedbackItemCells. 
    //We need to invalidate the model here to trigger a table reload. 
...
-(void) sendButtonWasClicked:(id)sender {
    ...
    _newFeedback.user_id = [NSNumber numberWithInt:17];  //[DBUser currentUser].user_id;
    ...
    _newFeedback.positive_vote_count = NULL;
    _newFeedback.negative_vote_count = NULL;
